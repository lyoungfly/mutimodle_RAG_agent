...
async function request(path, options = {}) {
  let response;
  try {response = await fetch(path, options);} catch {throw new Error('无法连接 PaperMind，请检查本地服务是否启动后重试。');}
  let data;
...
function renderDocuments() {
  ...
  for (const row of rows) {
    ...
    index.onclick = async () => {
      ...
      try {const result = await post('/documents/index',{document_id:row.document_id,collection_id:activeCollection});
        message('upload-status',`索引完成，共 ${result.chunk_count} 个证据块。${result.warnings.join('；')}`); await refresh();
    ...
    if (row.indexed) {
      ...
      view.onclick = async () => {
        ...
        try {const data = await request(`/documents/${row.document_id}/visuals?collection_id=${encodeURIComponent(activeCollection)}`);
          ...
          $('visual-panel').scrollIntoView({behavior:'smooth',block:'start'}); message('upload-status','');
        } catch (error) {message('upload-status',error.message,'error');} finally {lock(false);}
      };
      actions.append(view);
    }
  ...
  if (!rows.length) {
    const empty = element('li','','empty-state'); empty.append(element('strong',query ? '没有匹配的文献' : '你的下一个发现，从这里开始'),
      element('p',query ? '尝试其他文件名关键词。' : '添加第一份文献，PaperMind 会为它建立可追溯的证据索引。')); $('documents').append(empty);
  }
}
...
async function refreshCollections() {
  ...
  for (const name of new Set(['default',activeCollection,...collections.map(row => row.collection_id)])) {
...
$('upload-form').onsubmit = async event => {
  ...
  for (const [i,file] of files.entries()) {
    ...
    try {
      ...
      const indexed = await post('/documents/index',{document_id:uploaded.document_id,collection_id:activeCollection});
      row.textContent = `✓ ${file.name} · ${indexed.chunk_count} 个证据块${indexed.warnings.length ? ' · '+indexed.warnings.join('；') : ''}`; row.className = 'success'; completed++;
    } catch (error) {row.textContent = `× ${file.name}：${error.message}`; row.className = 'error';}
  }
  $('file').value = ''; message('upload-status',`处理完成：${completed}/${files.length} 份文献已就绪。${completed < files.length ? '已上传但未索引的文献可在下方重试。' : '返回研究工作台即可提问。'}`,completed === files.length ? 'success' : 'error');
...
$('chat-form').onsubmit = async event => {
  ...
  lock(true); message('query-status','正在查找相关证据并整理结果…','loading'); $('result-panel').hidden = true; lastResult = null;
  try {
    const type = $('type').value;
    const result = await post('/chat',{question,collection_id:activeCollection,mode:$('mode').value,filters:type ? {content_type:type} : null});
  ...
  } catch(error) {message('query-status',error.message,'error');} finally {lock(false);}
};
...
async function initialize() {
  ...
  try {
    ...
    await refresh(); await refreshCollections();
